...
main:		add %r0, %r0, %r4	!Clear r4
		...
		ld [%r4 + CICTL], %r1	!Load CICTL
		or %r1, 0x20, %r1	!Set enable interrupt bit
		st %r1, [%r4 + CICTL]	!Store CICTL
...
Loop:		inc %r31
		...
		ld [%r4 + COSTAT], %r1	!Load COSTAT
		or %r1, 0x20, %r1	!Set enable interrupt bit
		st %r1, [%r4, COSTAT]	!store COSTAT
		ba Loop
...
!KeyBoard Trap and ISR:
		...
		.org Keyboard_ISR
		ld [%r4 + CIN], %r2	!Load waiting char
...
CRT_Enter:	add %r0, Buff, %r2	!Reset buffer pointer
		st %r2,[BuffPtr]
		ld [%r4, CICTL], %r2	!Load CICTL and enable interrupts
		or %r2, 0x20, %r2
		st %r2, [%r4, CICTL]
		ld [%r4, COSTAT], %r2	!Load COSTAT and disable interrupts
		not %r2, %r2
		or %r2, 0x20, %r2
		not %r2, %r2
		st %r2, [%r4, COSTAT]

CRT_End:	st %r3, [%r4 + COUT]	!Store char into COUT
		rett %r16, 4		!Exit ISR
