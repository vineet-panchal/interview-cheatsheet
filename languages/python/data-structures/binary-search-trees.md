...
**Binary Search Trees (BST)** are a fundamental data structure in computer science. They are binary trees where each node has at most two children, and for each node, all elements in its left subtree are less than the node, and all elements in its right subtree are greater than the node. This property allows for efficient searching, insertion, and deletion operations.

## Key Operations

- **Insert**: Add a new key to the BST while maintaining the BST property.
...
- **Find Min/Max**: Get the minimum or maximum value in the tree.
- **Validation**: Check if the tree is a valid BST.
...
50
    40
            ...
            25
        20
In-order traversal: [20, 25, 35, 40, 50, 60, 70, 80]
```

## Time Complexity

- **Insert**: O(h), where h is the height of the tree (O(log n) for balanced BST)
- **Search**: O(h)
- **Delete**: O(h)
- **Traversals**: O(n)
- **Height**: O(n) in worst case, O(log n) for balanced
