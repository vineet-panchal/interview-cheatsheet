'''
The Goal:
You are working with the Breast Cancer Wisconsin dataset, a built-in dataset from scikit-learn.
It contains measurements of cell nuclei (features) from breast cancer biopsies.
Each record (sample) is labeled as benign (non-cancerous) or malignant (cancerous).
...
Part 3:
- Manually grow a small decision tree by hand (using a simple table and entropy/gain).
